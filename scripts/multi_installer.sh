...
_fail() {
    ...
    _colorize red "Please check build.log and if you need help check out the team discord @ 'https://discordapp.com/invite/A6Z5mfT'" && echo
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/evolution]; then
            read -r -p "${1:-evolution directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old evolution directory" && echo
                    rm -rf "$PWD"/evolution
                    ;;
                *)
                    _fail "evolution directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir evolution
        git clone -b master -q https://github.com/evolutioncoin/evolution evolution   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd evolution
    fi
}

_build_evolution() {
    _note "Building evolution from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/evolutioncoin')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/evolutioncoin')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "Evolution Multi_Installer v1.0 (pepperoni)"
_colorize green "ASCII ART NEEDS TO BE ADDED HERE" && echo
...
_note "Look in 'evolution/build/src/' for the executible binaries. See 'https://github.com/evolutioncoin/evolution' for more project support. Cowabunga!"
